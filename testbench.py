...
def test(taskid=1, iternum=10000, alpha=1):
    if taskid == 1:
        ...
        x0 = 1
        n = 4
        A = np.array([[1, -1, 1, 1], [-1, 2, -1, -1], [2, -1, 2, 1], [-1, -1, 1, 1]])
        b = np.array([2, -2, 4, 0])
        ...
        for comb in range(len(x1)):
            ...
            for t in range(N):
                ...
                wavg += np.matmul(
                    np.matrix(bm.value_nodes).T, np.matrix(bm.value_nodes)
                )
    ...
    if taskid != 2:
        ...
        for idx in range(n):
            ...
            enwave_t2.append(calc_energy(w, bm.value_nodes, n, theta, x0, C))
        plt.plot(enwave_t2)
        print(enwave_t2)
    bm.show()
    bm.view_graph()
